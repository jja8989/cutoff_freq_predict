...
def main_load_and_config():
    ...
    ErrorSearchRange = np.linspace(-tap, tap, 200)

    OSCMSE = np.zeros(len(ErrorSearchRange))

    for j in range(len(ErrorSearchRange)):
        offset = ErrorSearchRange[j]
        OSCMSE[j] = mean_squared_error(ch1_return, offset * np.ones_like(ch1_return))

    min_index = np.argmin(OSCMSE)
    
    OSCErrorMean = ErrorSearchRange[min_index]
    ...
    OSCErrorMean_original = OSCErrorMean
    ...
    plt.title("Figure 1. FFT Magnitude Spectrum (Linear scale)")
    ...
    plt.title("Figure 2. MSE Curves for Cutoff Frequency Search")
    plt.legend()
    plt.grid(True, which='both')
    plt.tight_layout()
    
    plt.figure(figsize=(8, 5))
    plt.plot(10 ** FreqSearchRange, np.sign(error_change_rate) * np.log(np.abs(error_change_rate)))
    # plt.plot(10 ** FreqSearchRange, original_grad)
    plt.plot(10 ** FreqSearchRange, log_original)
    plt.xscale('log')
    ax = plt.gca()
    ...
    plt.xlabel("Frequency [Hz]")
    plt.ylabel("Gradient Inverse (error change rate)")
    plt.title("Figure 3. Inverse Gradient Curve for Cutoff Frequency Detection")
    plt.legend()
    plt.grid(True, which='both')
    plt.tight_layout()
    ...
    plt.title("Figure 4. Time-domain Signal Before and After FFT Denoising")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    
    plt.figure(figsize=(8, 5))
    plt.plot(ErrorSearchRange, OSCMSE, 'b-')
    plt.axvline(OSCErrorMean_original, color='r', linestyle='--', label=f"Offset Mean = {OSCErrorMean_original:.3e}")
    plt.xlabel("Offset Value [V]")
    plt.ylabel("MSE")
    plt.title("Figure 5. DC Offset Optimization Curve")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    while state > 0:
        if state == 1:
            ...
            Q_charge_indic = f"Q_charge = {Q_charge:.3e} [C/m²]"
            Q_discharge_indic = f"Q_discharge = {Q_discharge:.3e} [C/m²]"
            Q_res_indic = f"Q_res = {Q_res:.3e} [C/m²]"
            Area_indic = f"Area_cap = {cap_area * 1e12:.2e} [μm²]"
            ErrorRate_indic = f"Q_res size = {ErrorRatio:.3f}% of Q_charge"
            
            MaxFreq_indic = f"Max FFT freq. = {DefInput:.2e}"
            FreqIncrement_indic = f"FFT freq. inc. = {1/T:.2e}"
            CutOffFreq_indic = f"CutOff freq. = {cutoff_freq:.2e}"
            ...
            ax[0].text(0.6 * time[-1], max(ch1), 'FFT info.', fontsize=8)
            ax[0].text(0.6 * time[-1], 0.9 * max(ch1), MaxFreq_indic, fontsize=8)
            ax[0].text(0.6 * time[-1], 0.8 * max(ch1), FreqIncrement_indic, fontsize=8)
            ax[0].text(0.6 * time[-1], 0.7 * max(ch1), CutOffFreq_indic, fontsize=8)
            ...
            ax[1].text(0.6 * time[-1], 0.6 * Q_charge, ErrorRate_indic, fontsize=8)
            ax[1].text(0.6 * time[-1], 0.7 * Q_charge, Q_res_indic, fontsize=8)
            ax[1].text(0.6 * time[-1], 0.8 * Q_charge, Q_discharge_indic, fontsize=8)
            ax[1].text(0.6 * time[-1], 0.9 * Q_charge, Q_charge_indic, fontsize=8)
            ax[1].text(0.6 * time[-1], Q_charge, Area_indic, fontsize=8)
        ...
        if state == 2:
            Q_charge_indic = f"Q₍charge₎ = {NewQ_charge:.3e} [C/m²]"
            Q_discharge_indic = f"Q₍discharge₎ = {NewQ_discharge:.3e} [C/m²]"
            Q_res_indic = f"Q₍res₎ = {NewQ_res:.3e} [C/m²]"
            Area_indic = f"Area₍cap₎ = {cap_area * 1e12:.3e} [μm²]"
            ErrorRate_indic = f"Q_res size = {ErrorRatio:.3f}% of Q₍charge₎"
            MaxFreq_indic = f"Max FFT freq. = {DefInput:.3e}"
            FreqIncrement_indic = f"FFT freq. inc. = {1/T:.3e}"
            CutOffFreq_indic = f"CutOff freq. = {cutoff_freq:.3e}"
            ...
            ax[0].plot(time, np.ones_like(time) * OSCErrorMean, 'b--', label='Offset')
            ax[0].plot(time, ch1_return, 'r-', linewidth=2, label='Denoised')
            ax[0].text(0.5 * time[-1], max(ch1), 'FFT info.', fontsize=8)
            ax[0].text(0.5 * time[-1], 0.9 * max(ch1), MaxFreq_indic, fontsize=8)
            ax[0].text(0.5 * time[-1], 0.8 * max(ch1), FreqIncrement_indic, fontsize=8)
            ax[0].text(0.5 * time[-1], 0.7 * max(ch1), CutOffFreq_indic, fontsize=8)
            ax[0].set_xlabel('Time [s]', fontsize=18, fontweight='bold')
            ax[0].set_ylabel('Current × 50Ω [V]', fontsize=18, fontweight='bold')
            ax[0].set_title('OSC measurment profile, noise removed', fontsize=15)
            ax[0].grid(True)
    ...
    fig.savefig(full_path, format='jpg', dpi=300, bbox_inches='tight')
